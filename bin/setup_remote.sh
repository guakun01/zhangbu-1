...
db_container_name=db-for-zhangbu
...
if [ "$(docker ps -aq -f name=^${DB_HOST}$)" ]; then
  ...
  docker run -d --name $DB_HOST \
            --network=network1 \
            -e POSTGRES_USER=zhangbu-1 \
            -e POSTGRES_DB=zhangbu_production \
            -e POSTGRES_PASSWORD=$DB_PASSWORD \
            -e PGDATA=/var/lib/postgresql/data/pgdata \
            -v zhangbu-data:/var/lib/postgresql/data \
            postgres:14
  echo '创建成功'
fi

title 'docker build'
docker build $root -t zhangbu:$version
